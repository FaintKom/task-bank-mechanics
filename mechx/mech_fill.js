...
function FractionShadeDemo() {
  ...
  const score = (c, v) => ({
    correct: (v?.length ?? 0) === c.target_filled
  });
  const SIZE = 240,
    R = SIZE / 2;
  const sectorPath = (k, n) => {
    const a0 = k / n * 2 * Math.PI - Math.PI / 2,
      a1 = (k + 1) / n * 2 * Math.PI - Math.PI / 2;
  ...
  return /*#__PURE__*/React.createElement(MechHost, {
    ...
    render: ({
      ...
      const filled = new Set(value ?? []);
      const parts = Array.from({
        length: config.denominator
      }, (_, k) => k);
      const toggle = i => {
        if (graded) return;
        const next = filled.has(i) ? [...filled].filter(x => x !== i) : [...filled, i];
      ...
      }, parts.map(k => /*#__PURE__*/React.createElement("path", {
        key: k,
        d: sectorPath(k, config.denominator),
        onClick: () => toggle(k),
        fill: filled.has(k) ? "var(--sel-accent)" : "var(--paper)",
        stroke: "var(--line)",
        strokeWidth: 3,
        style: {
      ...
      }, correct ? /*#__PURE__*/React.createElement(IconCheck, {
        s: 26
      ...
      }, filled.size, " / ", config.denominator), /*#__PURE__*/React.createElement("span", {
        className: "mech-label"
      }, "\u043D\u0443\u0436\u043D\u043E ", config.target_filled)));
    }
  });
}
...
function ColumnAddDemo() {
  ...
  const score = (c, v) => {
    const entered = (v ?? []).join("").trim();
    return {
      correct: entered !== "" && Number(entered) === c.a + c.b
    };
  };
  ...
  return /*#__PURE__*/React.createElement(MechHost, {
    config: config,
    score: score,
    initial: Array(n).fill(""),
    canCheck: v => (v ?? []).some(d => d !== ""),
    render: ({
      value,
      onChange,
      display
      ...
      const graded = display === Display.Graded;
      const aStr = String(config.a),
        bStr = String(config.b),
        sumStr = String(config.a + config.b);
      const cols = Array.from({
        length: n
      }, (_, j) => j);
      const digits = value ?? Array(n).fill("");
      const charAt = (s, j) => {
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        const state = graded ? (digits[j] ?? "") === expected ? "correct" : "wrong" : undefined;
...
function TableGridDemo() {
  ...
  const score = (c, v) => {
    const e = v ?? {};
    return {
      correct: c.rows.every((row, r) => row.cells.every((cell, ci) => !cell.blank || match(cell.value, e[key(r, ci)])))
    };
  };
  return /*#__PURE__*/React.createElement(MechHost, {
    config: config,
    ...
    canCheck: v => {
      ...
      return blanks.every(k => (v ?? {})[k] && (v ?? {})[k].trim() !== "");
    },
    render: ({
      value,
      onChange,
      ...
      }, config.headers.length > 0 && /*#__PURE__*/React.createElement("thead", null, /*#__PURE__*/React.createElement("tr", null, config.headers.map((h, c) => /*#__PURE__*/React.createElement("th", {
        key: c,
        style: {
          ...tdBase,
          padding: "6px 10px",
