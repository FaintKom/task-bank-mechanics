...
function DuplicateDemo() {
  ...
  return (
    <React.Fragment>
      ...
      <div className="demo-note">Две плитки «20» одинаковы. Проверка идёт по значениям: порядок «10 20 20 30» засчитывается независимо от того, какая из двух «20» где стоит. Для упорядочивания дубли уже работают.</div>
    </React.Fragment>
  );
}
...
const CASES = [
  ...
  { tag: "t-grade", tagText: "Задание · проверка", title: "Повторяющиеся значения: упорядочивание их уже принимает, вопрос про абакус", Demo: DuplicateDemo,
    ...
    fix: "Уточнение по коду: упорядочивание уже сверяет ответ по значениям, а не по тождеству плиток (последовательность меток === целевой), поэтому дубли засчитываются в любом их взаимном порядке, чинить здесь нечего. Случай Julia это абакус-задание на этой же механике: там id оказались привязаны к разрядам, и валидная случайная расстановка бусин помечается неверной. Базовое упорядочивание дубли по значению уже принимает, значит false-negative даёт привязка к id в самом задании. Нужна ссылка на конкретный таск от Julia, чтобы найти, где проверка цепляется за id вместо значения." },
];
