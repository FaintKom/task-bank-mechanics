...
function SmallSegmentsDemo() {
  ...
  return (
    <React.Fragment>
      <div className="ec-controls">
        <div className="seg">
          <button className={big ? "on" : ""} onClick={() => setBig(true)}>На 12 частей</button>
          <button className={!big ? "on" : ""} onClick={() => setBig(false)}>На 4 части</button>
        </div>
      </div>
      <TabletFrame><FractionShade key={big ? "b" : "s"} denominator={big ? 12 : 4} targetFilled={big ? 5 : 3} liveCounter /></TabletFrame>
      <div className="demo-note">{big
        ? "На 12 секторов доли узкие, особенно у центра круга, и палец легко задевает соседний сектор и меняет не ту часть."
        : "На 4 части сектора крупные, попасть в нужный легко."}</div>
    </React.Fragment>
  );
}
...
const CASES = [
  { tag: "t-age", tagText: "Возраст · фидбек", title: "Нет живого счётчика дроби", Demo: CounterDemo,
  ...
  { tag: "t-tab", tagText: "Планшет · эргономика", title: "Мелкие сектора и промах по соседней доле", Demo: SmallSegmentsDemo,
  ...
  { tag: "t-grade", tagText: "Задание · проверка", title: "«Закрась ноль» нельзя подтвердить", Demo: ZeroDemo,
