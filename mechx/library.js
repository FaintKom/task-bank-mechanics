...
(function () {
  ...
  var LIBRARY = {
    "choice": {
      ...
      vers: [["multiple.jpg", "Множественный выбор"], ["multiple-v3.jpg", "Композиция (v3)"]],
      ...
      warn: "Множественный выбор в макете визуально не отличить от одиночного: нет чекбокса или радиокнопки. Подсказки «выбери все подходящие» и счётчика «верно N из M» в макете нет."
    },
    "text-with-slots": {
      fig: "dropdown.jpg",
    ...
    "array-grid": {
      ...
      warn: "Макет и код расходятся. В макете задача «построй прямоугольник с заданным периметром»: углы перетаскиваются, проверяется периметр, произведения r×c нет. В коде это модель площади: тап строит r×c, проверяются стороны. Это разные модели, свести с командой дизайна."
    },
    "base10-blocks": {
      fig: null,
