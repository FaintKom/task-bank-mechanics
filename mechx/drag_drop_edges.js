...
const CASES = [{
  ...
  Demo: UnplacedDemo,
  problem: "«Проверить» доступна, как только разложена хотя бы одна карточка. Карточки, оставшиеся в лотке, грейдятся как ошибочные, хотя ребёнок их просто не разложил. «Не доделал» неотличимо от «положил не туда», и сигнала «разложи все» нет.",
  fix: "Доработать: блокировать «Проверить», пока лоток не пуст, или подсвечивать оставшиеся карточки перед проверкой; отличать «не разложено» от «неверно»."
}, {
  tag: "t-age",
