    ...
    <React.Fragment>
      <div className="ec-controls">
        <div className="seg">
          ...
          <button className={fix ? "on" : ""} onClick={() => setFix(true)}>флаг: ориентация не важна</button>
        </div>
      ...
      <div className="demo-note">{fix
        ? "Флаг orientationMatters=false: засчитываются обе ориентации, 4×3 принимается при цели 3×4. Для задач, где важна только площадь."
        : "Цель 3×4. Ребёнок строит 4×3, проверка различает ориентацию и не засчитывает. По умолчанию это правильно: для умножения порядок множителей важен."}</div>
    </React.Fragment>
  );
}
...
const CASES = [
  { tag: "t-grade", tagText: "Задание · логика", title: "3×4 и 4×3 различаются: верно для умножения, добавляем флаг для задач без порядка", Demo: CommutativeDemo,
