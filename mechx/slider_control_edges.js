...
const CASES = [{
  ...
  fix: "Доработать: укрупнять шаг под диапазон (round-числа), либо давать допуск; для точного выбора добавить кнопки −/+ для тонкой подстройки рядом с ползунком. Дать возможность задавать шаг в конфиге под конкретную задачу."
}];
function Cases() {
