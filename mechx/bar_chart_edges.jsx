...
function ZeroBarDemo() {
  return (
    <React.Fragment>
      <div className="ec-controls"><span className="mech-label" style={{ fontSize: 13 }}>У столбика «Ср» ответ 0, построй только Пн и Вт, проверь</span></div>
      <TabletFrame><BarChart max={5} bars={[{ label: "Пн", target: 3 }, { label: "Вт", target: 4 }, { label: "Ср", target: 0 }]} /></TabletFrame>
      <div className="demo-note">Незаполненный столбик считается нулём. Если target=0, он засчитывается верным, хотя ребёнок к нему вообще не прикоснулся, и нет подтверждения, что ноль выбран осознанно.</div>
    </React.Fragment>
  );
}
...
const CASES = [
  ...
  { tag: "t-grade", tagText: "Задание · проверка", title: "Нулевой столбик проходит без действия", Demo: ZeroBarDemo,
